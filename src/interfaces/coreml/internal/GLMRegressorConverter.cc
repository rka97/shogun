...
template<>
const std::unordered_set<std::string> GLMRegressorConverterType::supported_types = {"LibLinearRegression", "LeastAngleRegression", "LeastSquaresRegression", "LinearRidgeRegression"};

GLMRegressorConverter::GLMRegressorConverter(const CMachine* m): GLMRegressorConverterType(m)
{
	set_model_interface();

	m_spec->set_specificationversion(SPECIFICATION_VERSION);
	m_spec->set_allocated_glmregressor(CoreMLConverter::convert(static_cast<const input_type*>(m)));
}

template<>
void GLMRegressorConverterType::convert(const CLinearMachine* lr, Specification::GLMRegressor* spec)
{
